...
group('apiBaseQuery', () => {
  ...
  test('no auth header when no token', ({ expect }: any) => {
    const result = flushAsync(store.dispatch(api.endpoints.getData.initiate()));
    expect(result.data).toEqual({ result: 'ok' });
  });

  test('cache is populated after query', ({ expect }: any) => {
