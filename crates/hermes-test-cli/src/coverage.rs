...
pub fn instrument_bundle_with_sourcemap(
    source: &str,
    filename: &str,
    sm_info: &SourceMapInfo,
) -> Option<(String, String)> {
    instrument_bundle_inner(source, filename, Some(sm_info))
}

fn instrument_bundle_inner(
    source: &str,
    filename: &str,
    sm_info: Option<&SourceMapInfo>,
) -> Option<(String, String)> {
    ...
    for (i, (s, e, _)) in stmts.iter().enumerate() {
        if let Some((file, src_line, src_col)) = find_file_and_line(*s) {
            ...
            let (el, ec) = end_loc
                .map(|(_, l, c)| (l, c))
                .unwrap_or((src_line, src_col + (e - s)));
    ...
    for (file, stmts_list) in &file_stmts {
        let sm = build_loc_map_resolved(
            stmts_list.iter().map(|(gid, _, _)| *gid).collect(),
            &stmt_locs,
        );
        let fns_list = file_fns.get(file).cloned().unwrap_or_default();
        let fm = build_fn_map_resolved(
            fns_list
                .iter()
                .map(|(gid, _, _, n)| (*gid, n.clone()))
                .collect(),
            &fn_locs,
        );
        let br_list = file_br.get(file).cloned().unwrap_or_default();
        let bm = build_branch_map_resolved(
            br_list
                .iter()
                .map(|(gid, _, _, t, _)| (*gid, t.clone()))
                .collect(),
            &br_locs,
        );
        let s_ids: Vec<String> = stmts_list
            .iter()
            .map(|(gid, _, _)| gid.to_string())
            .collect();
        let f_ids: Vec<String> = fns_list
            .iter()
            .map(|(gid, _, _, _)| gid.to_string())
            .collect();
        let b_ids: Vec<String> = br_list
            .iter()
            .map(|(gid, _, _, _, _)| gid.to_string())
            .collect();
    ...
    let preamble = format!(
        "function __cov(){{var c=__cov.__c;if(c)return c;var g=globalThis;if(!g.__coverage__)g.__coverage__={{}};if(!g.__coverage__[\"{fid}\"])g.__coverage__[\"{fid}\"]={{path:\"{fid}\",s:new Int32Array({s_len}),f:new Int32Array({f_len}),b:{bi}}};__cov.__c=g.__coverage__[\"{fid}\"];return __cov.__c}}__cov();\n",
        s_len = stmts.len(),
        f_len = fns.len(),
    );
    ...
    for (o, c) in &merged {
        let p = *o as usize;
        if p >= last && p <= source.len() {
            out.push_str(&source[last..p]);
            out.push_str(c);
            last = p;
        }
    }
...
fn walk_iife(
    ...
    match expr {
        ...
        _ => {
            walk_expr(expr, stmts, fns, fbo, br);
        }
    }
}

fn is_bare(s: &Statement) -> bool {
...
fn walk_stmt(
    ...
    match stmt {
        ...
        Statement::ClassDeclaration(c) => {
            stmts.push((sp.start, sp.end, bare));
            walk_class(&c.body, stmts, fns, fbo, br);
        }
        Statement::IfStatement(i) => {
            stmts.push((sp.start, sp.end, bare));
            br.push((sp.start, sp.end, "if".into(), 2));
            walk_stmt(&i.consequent, stmts, fns, fbo, br, is_bare(&i.consequent));
            if let Some(a) = &i.alternate {
                walk_stmt(a, stmts, fns, fbo, br, is_bare(a));
            }
        }
        ...
        Statement::SwitchStatement(sw) => {
            stmts.push((sp.start, sp.end, bare));
            br.push((sp.start, sp.end, "switch".into(), sw.cases.len() as u32));
            for c in &sw.cases {
...
fn walk_expr(
    ...
    match expr {
        Expression::CallExpression(c) => {
            walk_expr(&c.callee, stmts, fns, fbo, br);
        ...
        Expression::ArrowFunctionExpression(a) => {
            let sp = a.span;
            fns.push((sp.start, sp.end, "(arrow)".into()));
            if a.expression {
                fbo.push(None);
                for s in &a.body.statements {
                    if let Statement::ExpressionStatement(e) = s {
                ...
                fbo.push(Some(a.body.span.start + 1));
                for s in &a.body.statements {
                    walk_stmt(s, stmts, fns, fbo, br, false);
                }
            }
        ...
        Expression::ArrayExpression(a) => {
            for e in &a.elements {
                if let ArrayExpressionElement::SpreadElement(s) = e {
                    walk_expr(&s.argument, stmts, fns, fbo, br);
                } else if let Some(e) = e.as_expression() {
                    walk_expr(e, stmts, fns, fbo, br);
                }
            }
        }
...
fn walk_class(
    ...
    for el in &body.body {
        if let ClassElement::MethodDefinition(m) = el {
            let n = if let PropertyKey::StaticIdentifier(i) = &m.key {
                i.name.to_string()
            } else {
                "(method)".into()
            };
            fns.push((m.span.start, m.span.end, n));
...
impl LineTable {
    ...
    fn lookup(&self, o: u32) -> (u32, u32) {
        let l = match self.line_starts.binary_search(&o) {
            Ok(i) => i,
            Err(i) => i.saturating_sub(1),
        };
        (l as u32 + 1, o.saturating_sub(self.line_starts[l]))
    }
}
fn build_loc_map(l: &[(u32, u32)], lt: &LineTable, base: u32) -> String {
    let mut o = String::from("{");
    for (i, (s, e)) in l.iter().enumerate() {
        let (sl, sc) = lt.lookup(*s);
        let (el, ec) = lt.lookup(*e);
        let sl = sl.saturating_sub(base);
        let el = el.saturating_sub(base);
        if i > 0 {
            o.push(',');
        }
...
fn build_fn_map(f: &[(u32, u32, String)], lt: &LineTable, base: u32) -> String {
    let mut o = String::from("{");
    for (i, (s, e, n)) in f.iter().enumerate() {
        let (sl, sc) = lt.lookup(*s);
        let (el, ec) = lt.lookup(*e);
        let sl = sl.saturating_sub(base);
        let el = el.saturating_sub(base);
...
fn build_branch_map(b: &[(u32, u32, String, u32)], lt: &LineTable, base: u32) -> String {
    let mut o = String::from("{");
    for (i, (s, e, t, _)) in b.iter().enumerate() {
        let (sl, sc) = lt.lookup(*s);
        let (el, ec) = lt.lookup(*e);
        let sl = sl.saturating_sub(base);
        let el = el.saturating_sub(base);
...
fn build_loc_map_resolved(
    gids: Vec<usize>,
    locs: &BTreeMap<usize, (u32, u32, u32, u32)>,
) -> String {
    let mut o = String::from("{");
...
fn build_zero_map(c: usize) -> String {
    let mut o = String::from("{");
    for i in 0..c {
        if i > 0 {
            o.push(',');
...
fn build_branch_zero_map(b: &[(u32, u32, String, u32)]) -> String {
    let mut o = String::from("{");
    for (i, (_, _, _, p)) in b.iter().enumerate() {
        if i > 0 {
            o.push(',');
        }
        let z: Vec<&str> = (0..*p).map(|_| "0").collect();
...
pub fn print_summary(lcov: &str) -> f64 {
    ...
    for (name, h, t, fh, ft) in &files {
        ...
        let display_name = if name.len() > max_name {
            &name[name.len() - max_name..]
        } else {
            name
        };
        eprintln!(
            " {:<width$}  {color}{:>5.1}%\x1b[0m  {fn_color}{:>5.1}%\x1b[0m",
            display_name,
            pct,
            fn_pct,
            width = max_name
    ...
    let total_fn_pct = if total_fns > 0 {
        (total_fn_hit as f64 / total_fns as f64) * 100.0
    } else {
        100.0
    };
    let color = if total_pct >= 80.0 {
        "\x1b[32m"
    ...
    let fn_color = if total_fn_pct >= 80.0 {
        "\x1b[32m"
    } else if total_fn_pct >= 50.0 {
        "\x1b[33m"
    } else {
        "\x1b[31m"
    };
...
"#,
    ...
    let file_sources: Vec<Vec<String>> = files
        .iter()
        .map(|(name, _)| {
    ...
    let avg_pct = if !file_pcts.is_empty() {
        file_pcts.iter().sum::<f64>() / file_pcts.len() as f64
    } else {
        100.0
    };
    let full_covered = file_pcts.iter().filter(|p| **p >= 100.0).count();

    let pct_color = |p: f64| -> &str {
        if p >= 80.0 {
            "#0f9d58"
        } else if p >= 50.0 {
            "#f4b400"
...
pub fn collect_coverage(
    rt: &crate::engine::Runtime,
    map_path: Option<&std::path::Path>,
) -> Option<String> {
    let js = "globalThis.__coverage__";
    ...
    if let Some(p) = map_path {
        if let Ok(ms) = std::fs::read_to_string(p) {
            if let Ok(file_map) = serde_json::from_str::<serde_json::Value>(&ms) {
                // Get runtime counters from the first coverage bucket. Some engines may
                // serialize typed-array counters differently (object-like or array-like),
                // so we normalize lookups instead of requiring one exact shape.
                let bundle_key = runtime.as_object().and_then(|o| o.keys().next().cloned());
                let rt_data = bundle_key
                    .as_ref()
                    .and_then(|k| runtime.get(k))
                    .unwrap_or(&runtime);
                let rt_s = rt_data.get("s");
                let rt_f = rt_data.get("f");
                let rt_b = rt_data.get("b");
                if rt_s.is_none() || rt_f.is_none() || rt_b.is_none() {
                    return Some(coverage_to_lcov(&runtime));
                }
                let rt_s = rt_s.unwrap();
                let rt_f = rt_f.unwrap();
                let rt_b = rt_b.unwrap();

                let counter_lookup =
                    |counters: &serde_json::Value, key: &str| -> Option<serde_json::Value> {
                        match counters {
                ...
                let gid_key = |gid: &serde_json::Value| -> String {
                    if let Some(s) = gid.as_str() {
                        s.to_string()
                    } else if let Some(n) = gid.as_u64() {
                        n.to_string()
                    } else {
                        gid.to_string().replace('"', "")
                    }
                };

                // Build per-file coverage entries
                let mut per_file = serde_json::Map::new();
                for (file, fdata) in file_map.as_object().into_iter().flatten() {
                    let mut entry = serde_json::Map::new();
                    entry.insert("path".into(), serde_json::Value::String(file.clone()));

                    // Copy statementMap, fnMap, branchMap from disk
                    if let Some(v) = fdata.get("statementMap") {
                        entry.insert("statementMap".into(), v.clone());
                    }
                    if let Some(v) = fdata.get("fnMap") {
                        entry.insert("fnMap".into(), v.clone());
                    }
                    if let Some(v) = fdata.get("branchMap") {
                        entry.insert("branchMap".into(), v.clone());
                    }
                    ...
                    if let Some(ids) = fdata["_sIds"].as_array() {
                        ...
                        for (local, gid) in ids.iter().enumerate() {
                            let key = gid_key(gid);
                            let val = counter_lookup(rt_s, &key)
                    ...
                    if let Some(ids) = fdata["_fIds"].as_array() {
                        ...
                        for (local, gid) in ids.iter().enumerate() {
                            let key = gid_key(gid);
                            let val = counter_lookup(rt_f, &key)
                                .unwrap_or(serde_json::Value::Number(0.into()));
                    ...
                    if let Some(ids) = fdata["_bIds"].as_array() {
                        ...
                        for (local, gid) in ids.iter().enumerate() {
                            let key = gid_key(gid);
                            let val = counter_lookup(rt_b, &key).unwrap_or(serde_json::json!([]));
    ...
    for (f, fc) in o {
        ...
        if let Some(sm) = fc["statementMap"].as_object() {
            if let Some(s) = fc["s"].as_object() {
                ...
                for (id, st) in sm {
                    let li = st["start"]["line"].as_u64().unwrap_or(0);
                    let c = s.get(id).and_then(|v| v.as_u64()).unwrap_or(0);
                    let e = lines.entry(li).or_insert(0);
                    *e = (*e).max(c);
                }
                for (li, c) in &lines {
                    l.push_str(&format!("DA:{li},{c}\n"));
                }
            }
        }
            ...
            if let Some(b) = fc["b"].as_object() {
                for (id, bd) in bm {
                    if let Some(cs) = b.get(id).and_then(|v| v.as_array()) {
                        let li = bd["loc"]["start"]["line"].as_u64().unwrap_or(0);
